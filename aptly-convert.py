...
# You can use the script for any directory structure containing DEB files (such as the default
# mirror directory "/var/spool/apt-mirror" for DEB files mirrored with apt-mirror).
# After executing this script the new directory structure of the files will be compatible with aptly.
    ...
    for (path, dirs, files) in os.walk(src_path):
        ...
        for f in files:
            ...
            with open(current_file,"rt") as FIN:
                H.update(FIN.buffer.read())

            current_sha256 = H.hexdigest()
            current_dst_path =  dst_path + '/' + current_sha256[:2] + '/' + current_sha256[2:4] + '/'

            if args.verbosity == "2":
                print ("File:\t" + current_file + "\tSHA256SUM:\t" + current_sha256 + "\tDest. Path:\t" + current_dst_path )
            ...
            current_dst = os.path.abspath (current_dst_path + current_sha256[5:31] + "_" + os.path.basename(current_file))
